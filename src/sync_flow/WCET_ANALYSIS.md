# WCET Analysis: SyncFlow Federated Program

## Executive Summary

This document provides a detailed Worst-Case Execution Time (WCET) analysis for the SyncFlow federated program running across three FRDM-K64F microcontrollers. The program implements a two-branch barrier synchronization pattern with a chained top path.

**Key Findings:**
- **End-to-End WCET per Round**: ~750 ms (worst case)
- **Critical Path**: Top Chain (two sequential workers with quadratic workload)
- **Bottleneck**: Top Chain computation + network delays (470ms + network overhead)
- **Throughput**: ~1.25 rounds/second (with 800 ms spacing)
...
└───────┼─────────────────────┼──────────────────────────────┘
        │                     │
        │ 20ms                │ 500ms (from Top2)
        │                     │ 180ms (from Bottom)
        ▼                     ▼
┌───────────────────────────┐       ┌───────────────┐
│ Top Chain Federate        │       │ Bottom Worker │
...
|------------|-------------|----------|---------|
| Host → Top Chain | 20 ms | 100 ms | Task distribution |
| Top Chain → Host | 500 ms | 550 ms | Result collection (after both Top1 and Top2) |
| Host → Bottom | 20 ms | 100 ms | Task distribution |
| Bottom → Host | 180 ms | 250 ms | Result collection |
