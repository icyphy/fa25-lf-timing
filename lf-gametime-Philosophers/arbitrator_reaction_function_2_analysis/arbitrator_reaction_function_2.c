...
// Reactor: arbitrator_reaction_function_2
// Generated from LF compiler output

void _arbitratorreaction_function_2(size_t* num_philosophers, bool** forks, int** replies, size_t* finished_philosophers, size_t* arbitration_id, size_t* retries) {
    for(size_t i = 0; i < *num_philosophers; i++) {
            if (*replies[i] == EAT) {
                lf_set(eat[i], true);
            } else if (*replies[i] == DENIED) {
