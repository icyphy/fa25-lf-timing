...
// Reactor: arbitrator_reaction_function_1
// Generated from LF compiler output

void _arbitratorreaction_function_1(size_t* num_philosophers, bool** forks, int** replies, size_t* finished_philosophers, size_t* arbitration_id, size_t* retries) {
    (*finished_philosophers) = 0;
        (*retries) = 0;
        (*arbitration_id) = 0;
        memset(*replies, INVALID, sizeof(int) * *num_philosophers);
        memset(*forks, INVALID, sizeof(bool) * *num_philosophers);
}
