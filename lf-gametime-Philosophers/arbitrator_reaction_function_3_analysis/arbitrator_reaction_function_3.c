...
// Reactor: arbitrator_reaction_function_3
// Generated from LF compiler output

void _arbitratorreaction_function_3(size_t* num_philosophers, bool** forks, int** replies, size_t* finished_philosophers, size_t* arbitration_id, size_t* retries) {
    for(size_t i = 0; i < done_width; i++) {
            if (done[i]->is_present) {
                free_forks(*forks, i, *num_philosophers);
            }
        }
}
