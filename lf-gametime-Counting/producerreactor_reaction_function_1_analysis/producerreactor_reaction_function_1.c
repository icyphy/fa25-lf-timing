...
// Reactor: producerreactor_reaction_function_1
// Generated from LF compiler output

// Scheduling counter for WCET analysis
int schedule_count = 0;

void _producerreactorreaction_function_1(int* countTo, int* i) {
    bool __output_outIncrement;
    bool __output_outRetrieveResult;
    if (*i < *countTo) {
            __output_outIncrement = true;;
            (*i)++;
            schedule_count++;  // lf_schedule overhead for WCET
        } else {
            __output_outRetrieveResult = true;;
        }
}
